...
which(is.na(BikeSharing$wt_rain))
which(is.na(BikeSharing$precip))
```
...
vec2 <- c("total_cust", "temp_min", "temp_max", "temp_observ", "precip", "wind")
...
# Now we can plot a matrix of scatter plots for the cts variables
# We can observe that total_cust or dependent variable has strong correalation with Highly significant (p-value < 0.001) indicated by the three stars with temp_min, temp_max, temp_observ, precip, and wind. From this plot an associated decrease in precip and wind would increase total customers. This is intuititive as people would genrally rent out bikes as the weather has less wind and precipitation.
...
```
VARS MODEL Did Better ASE wise for 6 month forecast than the ARMA(1,1) S =7  model.
```{r}

library(vars)

# Remove rows with NA values from the BikeSharing dataset
BikeSharing <- na.omit(BikeSharing)

n_rows <- nrow(BikeSharing)
print(n_rows)
...
#run the code below with function VARselect to find a p to fit into Var Function

VAR_Bikeshare2 = VAR(cbind(TotalCust,Precipitation,Wind,Tempe_Observe,Temp_Min,Temp_Max),lag.max = 10, type = "both",p = 9)

VAR_Bikeshare2

#make predictions with model with 182 prediction (6months)
preds=predict(VAR_Bikeshare2,n.ahead=182)

preds

#Plot
plot(BikeSharing$total_cust, type = "l",xlim = c(0,2922), ylab = "Total Customers", main = "6 month Total Customer Forecast")
...
#Plot to show Forcast prediction up to 6 months
plot(seq(1,2922,1), BikeSharing$total_cust, type = "l",xlim = c(0,3104), ylab = "Total Customers", main = "6 month Total Customer Forecast")
...
#plot
plot(seq(1,1059,1),BikeSharing$total_cust[1:1059], type = 'l',xlim = c(0,1070), ylab = "Total Customers", main = "6 month Total Customer Forecast")
