...
est.7.365=est.arma.wge(b7_365,p=8,q=0)

#est.7.365

est.7.365$res

#step 1 visual test => data seems to be stationary
plot.ts(est.7.365$res)

plotts.sample.wge(est.7.365$res)

# get the phis 
est.7.365$phi

#run ljung test to see if we should predict
ljung.wge(est.7.365$res,p=8, q=0)

ljung.wge(est.7.365$res,K=48,p=8, q=0)


#Forecasting with SARIMA model here such that ARIMA(p,d,q) with s=7
f = fore.arima.wge(BikeSharing$total_cust, s=7, phi = est.7.365$phi, n.ahead = 182, limits =  F, lastn = T, plot=TRUE)
...
#compare overfit estimated table with the season factor table of S =7
...
for( i in 1: sims)
{
   SpecDen2 = parzen.wge(gen.aruma.wge(319, s=7, phi = est.7.365$phi, theta =est.7.365$theta, plot ="FALSE"), plot = "FALSE")
...
for( i in 1: sims)
{
   ACF2 = acf(gen.aruma.wge(319, s = 7, phi = est.7.365$phi,theta =est.7.365$theta, plot =    "FALSE"), plot = "FALSE")
...
S10AR3gen = gen.aruma.wge(319,s = 7,phi = est.7.365$phi,theta =est.7.365$theta, vara = est.7.365$avar)
...
S10AR3gen <- gen.arima.wge(319, s = 7, phi = est.7.365$phi, vara = est.7.365$avar)
...
for(i in 1:sims) 
{
   ...
   S10AR3gen_new <- gen.arima.wge(319, s = 7, phi = est.7.365$phi, vara = est.7.365$avar, plot = "FALSE")
...
for(i in 1:sims) {
   S10AR3gen_new <- gen.arima.wge(319, s = 7, phi = est.7.365$phi, vara = est.7.365$avar, plot = "FALSE")
   lines(tail(S10AR3gen_new, 182), col = "red", lwd = 3) # Plot only the last 60 values
}
...
f6m = fore.arima.wge(BikeSharing$total_cust, s=7, phi = est.7.365$phi, n.ahead = 182, limits =  F, lastn = T, plot=TRUE)
...
# Sakava Alternative code with S = 7
f1w = fore.arima.wge(BikeSharing$total_cust, s=7, phi = est.7.365$phi, n.ahead = 7, limits =  F, lastn = T, plot=TRUE)
