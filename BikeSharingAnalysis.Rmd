...
# Prediction
pred_VAR = predict(VAR_BikeSharing, n.ahead = 182)
```
...
lines((nrow(BikeSharing) - 181):nrow(BikeSharing), pred_VAR$fcst$TotalCust[,1], col = "red")
```
...
ASE_VAR = mean((tail(BikeSharing$total_cust, 182) - pred_VAR$fcst$TotalCust[,1])^2)
print(ASE_VAR)
```
...
```{r}
fore_mlp = forecast(fit_mlp, h = 182, xreg = BikeSharingSelected[-(1:2300), -1])
```
...
```{r}
ASE_mlp = mean((tail(BikeSharing$total_cust, 182) - fore_mlp$mean)^2)
print(ASE_mlp)
```
...
```{r}
#Ensemble
ensemble = (preds_arma_182$f + pred_VAR$fcst$TotalCust[, 1] + fore_mlp$mean) / 3
...
lines(seq(457,508,1), ensemble, type = "l", col = "red")

ASE_ensemble = mean((tail(BikeSharing$total_cust, 182) - ensemble)^2)
ASE_ensemble
```
